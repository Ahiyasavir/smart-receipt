...
//
//   Case 3 — Israeli shekel (₪): "₪12,90" → "₪12.90" (same as Case 1 but with ₪)
function normalizeCommaDecimals(text: string): string {
  ...
  // Case 3: Israeli shekel prefix
  const hasShekelCommaPrice = /₪\s*\d+,\d{2}\b/.test(text);
  if (hasShekelCommaPrice && !hasPeriodPrice) {
    return text.replace(/(₪\s*\d+),(\d{2})\b/g, '$1.$2');
  }

  // Case 2: International comma-decimal (CZK, EUR, Israeli without ₪ prefix) without currency prefix.
  // Pattern: 1–4 digits, comma, exactly 2 digits, then an optional noise digit,
  // followed by whitespace / letter / bracket / Hebrew / end-of-line.
  ...
  if (commaCount >= 4 && commaCount > periodCount * 2) {
    return text.replace(/\b(\d{1,4}),(\d{2})\d?(?=[\s,A-Za-zא-ת)\]]|$)/gm, '$1.$2');
  }
